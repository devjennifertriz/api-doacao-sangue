...
class Doador(models.Model):
    ...
    tipo_sanguineo = models.CharField(
        max_length=3, choices=TIPO_SANGUINEO_CHOICES, verbose_name="Tipo Sanguíneo"
    )
...
class CriterioDoacao(models.Model):
    ...
    categoria = models.CharField(
        max_length=10, choices=CATEGORIA_CHOICES, default="REQ"
    )

    saiba_mais = models.TextField(
        blank=True, null=True, help_text="Informações detalhadas sobre este critério."
    )
...
class RegistrarDoacao(models.Model):
    doador = models.ForeignKey(
        "Doador", on_delete=models.CASCADE, related_name="registros"
    )
